...
describe("CollectorTest", function() {
  ...
  let wethSponsorAddress, daiSponsorAddress;
  ...
  it("deposit and push weth", async () => {
    ...
    await expect(weth.balanceOf(await collector.getAddress())).to.eventually.equal(ONE_ETHER / 2n);
  ...
  it("deposit and push dai", async () => {
    ...
    await expect(dai.balanceOf(await collector.getAddress())).to.eventually.equal(ONE_THOUSAND_DAI);
